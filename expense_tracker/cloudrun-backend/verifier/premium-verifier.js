...
function buildEntitlement({
  ...
  return {
    ...
    productId,
    packageName,
    // Keep both expiry fields because the Flutter premium cache reads them.
...
function toPublicEntitlementFields(entitlement) {
  ...
  return {
    ...
    orderId: entitlement?.latestOrderId ?? null,
...
function mapGoogleApiError(error, payload, verifierConfig) {
  ...
  const statusCode = extractGoogleStatusCode(sourceError);
  const rejectedReason = resolveRejectedGoogleReason(sourceError);
  const safeMeta = {
    googleStatusCode: statusCode || null,
    reason: rejectedReason || null,
  };

  if (error?.isAcknowledgementFailure) {
    logVerifierEvent('google_play_acknowledgement_failed', safeMeta, 'warn');
    return {
      statusCode: 503,
...
function isAlreadyAcknowledgedError(error) {
  ...
  const errorText = extractGoogleErrorText(error);
  return (
    errorText.includes('already acknowledged') ||
    errorText.includes('already been acknowledged') ||
    errorText.includes('subscription is already acknowledged') ||
    ((statusCode === 400 || statusCode === 409) &&
      errorText.includes('acknowled'))
  );
}
...
async function acknowledgeSubscriptionIfNeeded({
  ...
  try {
    ...
    if (isAlreadyAcknowledgedError(error)) {
      logVerifierEvent(
        'google_play_acknowledge_already_acknowledged',
        {
          productId: payload.productId,
          packageName: payload.packageName,
          tokenFingerprint: hashTokenFingerprint(payload.purchaseToken),
    ...
    // Fail closed when acknowledgement is still required but cannot be completed.
    logVerifierEvent(
    ...
    const acknowledgementError = new Error(
      RECOVERABLE_FAILURE_MESSAGE,
    );
    acknowledgementError.code = 'acknowledgement_failed';
    acknowledgementError.isAcknowledgementFailure = true;
    acknowledgementError.originalError = error;
...
async function verifyWithGooglePlayV2(androidpublisher, payload, verifierConfig) {
  ...
  const resolvedAcknowledgementState = await acknowledgeSubscriptionIfNeeded({
    androidpublisher,
    payload,
    acknowledgementState,
  });
  ...
  return buildAcceptedResponse(
    buildEntitlement({
      ...
      acknowledgementState:
        resolvedAcknowledgementState || acknowledgementState || null,
      latestOrderId,
...
async function verifyWithGooglePlayLegacy(
  ...
  return buildAcceptedResponse(
    buildEntitlement({
      ...
      message: DEFAULT_SUCCESS_MESSAGE,
      acknowledgementState:
        resolvedAcknowledgementState || acknowledgementState,
...
async function handlePremiumVerification(
  ...
  if (!verifierConfig.authConfigured) {
    return sendJson(
      res,
      500,
      buildErrorResponse({
        code: 'server_auth_not_configured',
        message: 'Premium verifier auth is not configured.',
        productId: verifierConfig.expectedProductId,
        packageName: verifierConfig.packageName,
        recoverable: false,
      }),
  ...
  const authMatched = timingSafeEqualString(
    providedToken,
    verifierConfig.sharedSecret,
  );
  logVerifierEvent('verify_premium_auth_resolved', {
    authMatched,
    hasAuthorizationToken: providedToken.length > 0,
    hasPurchaseToken: safeRequestMeta.hasPurchaseToken,
    tokenFingerprint: safeRequestMeta.tokenFingerprint,
    productId: safeRequestMeta.productId,
    packageName: safeRequestMeta.packageName,
    platform: safeRequestMeta.platform,
  });
  ...
  if (!providedToken) {
    return sendJson(
      ...
      buildErrorResponse({
        code: 'missing_auth',
        message: 'Missing verifier authorization.',
        productId: verifierConfig.expectedProductId,
        packageName: verifierConfig.packageName,
        recoverable: false,
      }),
  ...
  if (!authMatched) {
    return sendJson(
      ...
      buildErrorResponse({
        code: 'invalid_auth',
        message: 'Invalid verifier authorization.',
        productId: verifierConfig.expectedProductId,
        packageName: verifierConfig.packageName,
        recoverable: false,
      }),
