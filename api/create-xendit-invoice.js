export default async function handler(req, res) {
  ...
  try {
    ...
    if (!secretKey) return res.status(500).json({ error: "XENDIT_SECRET_KEY is not configured in Vercel Environment Variables." });
    ...
    const payload = {
      ...
      items: [{ name: packageName, quantity: 1, price: Number(amount), category: "Digital Services" }],
    ...
    if (!xenditResponse.ok) return res.status(xenditResponse.status).json({ error: data.message || "Xendit checkout creation failed." });
